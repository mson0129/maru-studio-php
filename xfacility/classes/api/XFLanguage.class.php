...
/**
 * # class XFLanguage
 * - This class is used to get the language information.
 * 
 * ## Contributors
 * - Studio2b
 ...
 * - 06NOV2015(1.0.3.) - Some bugs are fixed.
 */
class XFLanguage extends XFObject {
	...
	function getXFacilityLanguages() {
		require (parent::path()."/configs/XFLanguage.config.php");
	...
	function getApplicationLanguages($application) {
		...
		if(!is_null($application) && is_dir($_SERVER['DOCUMENT_ROOT'].$path)) {
			$dirClass = new XFDirectory($path);
			$temp = $dirClass->list_directory();
