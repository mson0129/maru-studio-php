...
/**
 * # class XFView
 * - This class is for managing views.
 * 
 * ## Contributors
 * - Studio2b
 ...
 * 
 * ## History
 * - 01JUL2014(1.0.0.) - This file is newly added.
 * - 19JUL2014(1.0.1.) - A bug of replace() is modified.
 * - 06FEB2015(1.0.2.) - A bug of replace() is modified.
 */
class XFView extends XFObject {
	public $view, $show;
	public $basePath, $application, $languages, $file;
	
	public function XFView($template=NULL, $replacements=NULL) {
		if(!is_null($template))
			$this->create($template, $replacements);
	}
	
	public function create($template, $replacements=NULL) {
		unset($this->view);
	...
	public function import($path=NULL) {
		...
		if(substr($path, 0, 1)=="/" && $file_flag==true) {
